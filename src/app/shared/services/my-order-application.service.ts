...
export class MyOrderApplicationService extends GridService {
  /**
   *
   */
  url = 'my-order-application';
  ...
  override mapToSetBaseResponse() {
    return map((result: BaseResponse<GridModel<SearchApplicationResponse>>) => {
      ...
      this.baseResponse = {
        ...
        data: {
          ...result.data,
          data: isFakeData ? result.data.data : this.baseResponse.data.data.concat(result.data.data),
        },
      };
